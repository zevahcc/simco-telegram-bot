...
async def get_market_prices():
    ...
    logger.info(f"Intentando obtener precios del mercado de: {API_URL}") # Nuevo log: inicio de la petición
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(API_URL, timeout=15.0) # Aumentar timeout por si acaso
            
            logger.info(f"Respuesta de la API: Estado {response.status_code}") # Nuevo log: estado de la respuesta
            response.raise_for_status()  # Lanza una excepción para códigos de estado HTTP 4xx/5xx

            data = response.json()
            if 'prices' in data:
                logger.info("Precios obtenidos con éxito de la API.") # Nuevo log: éxito
                return data['prices']
            else:
                logger.warning(f"La respuesta de la API no contiene la clave 'prices'. Respuesta completa: {data}") # Nuevo log: estructura inesperada
                return []
        ...
        logger.error(f"Error de estado HTTP al obtener precios de {API_URL}: {e.response.status_code} - {e.response.text}") # Detalle del error HTTP
        return None
    except json.JSONDecodeError as e:
        logger.error(f"Error al decodificar la respuesta JSON de la API. Puede que la respuesta no sea JSON válida: {e}")
        try:
            # Intenta loggear el contenido de la respuesta si no es JSON válido
            logger.error(f"Contenido de la respuesta (posiblemente no JSON): {response.text}")
        except NameError: # If response was not defined
            pass
        return None
    except Exception as e:
        logger.error(f"Error inesperado al obtener precios de {API_URL}: {e}", exc_info=True) # exc_info=True para el stack trace
        return None
